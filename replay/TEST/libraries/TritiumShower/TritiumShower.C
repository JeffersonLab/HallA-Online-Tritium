...
ClassImp(TritiumShower)

using namespace std;

//_____________________________________________________________________________
TritiumShower::TritiumShower( const char* name, const char* description,
		      THaApparatus* apparatus ) :
  THaShower(name,description,apparatus), fBlksptr(NULL)
{
  // Constructor.
  fBlocks = new TClonesArray("TritiumShowerBlock",200);
  fClusters = new TClonesArray("TritiumShowerCluster",10);
  fTrackProj = new TClonesArray("THaTrackProj",5);

}

//_____________________________________________________________________________
Int_t TritiumShower::ReadDatabase( const TDatime& date )
{
  ...
  *buf = '\0';
  ...
  //Double_t tan_angle = TMath::Tan(angle*degrad);
 // Double_t sin_angle = TMath::Sin(angle*degrad);
  //Double_t cos_angle = TMath::Cos(angle*degrad);
  ...
  fBlksptr = new TritiumShowerBlock*[fNelem];
  ...
  for( int c=0; c<ncols; c++ ) {
    for( int r=0; r<nrows; r++ ) {
      ...
      fBlockY[k] = y + c*dy;
      fBlksptr[k] = new TritiumShowerBlock(fBlockX[k],fBlockY[k],r,c,k);
      //new ((*fBlocks)[k]) TritiumShowerBlock(fBlockX[k],fBlockY[k],r,c,k);
    }
  }
  fclose(fi);
  return kOK;
...
//_____________________________________________________________________________
Int_t TritiumShower::DefineVariables( EMode mode )
{
  ...
  RVarDef vars[] = {
    ...
    { "e",      "Energy (MeV) of clusters",    "fClusters.TritiumShowerCluster.fE" },
    { "x",      "x-position (cm) of clusters", "fClusters.TritiumShowerCluster.fCenterX" },
    { "y",      "y-position (cm) of clusters", "fClusters.TritiumShowerCluster.fCenterY" },
    { "mult",   "Multiplicity of clusters",    "fClusters.TritiumShowerCluster.GetMult()" },
...
//_____________________________________________________________________________
inline
void TritiumShower::ClearLastEvent()
{
...
//_____________________________________________________________________________
Int_t TritiumShower::Decode( const THaEvData& evdata )
{
  ...
  // fAsum_c          -  Sum of shower blocks corrected ADC values;
    ...
    for (Int_t k=0; k<fNelem; k++) {
        TritiumShowerBlock* block = new ((*fBlocks)[k]) TritiumShowerBlock(fBlksptr[k]);
...
//_____________________________________________________________________________
Int_t TritiumShower::CoarseProcess( TClonesArray& tracks )
{
    ...
    TritiumShowerBlock* emaxblk = static_cast<TritiumShowerBlock*>(fBlocks->At(0));
    ...
    while (MaxBlockEnergy>fEmin) {
        ...
        TritiumShowerCluster* cluster = new ((*fClusters)[nextClust++]) TritiumShowerCluster;
        cluster->AddBlock(emaxblk);
        fBlocks->RemoveAt(0);

        for (Int_t i=1; i<fBlocks->GetLast()+1; i++) {
            TritiumShowerBlock* blk = static_cast<TritiumShowerBlock*>(fBlocks->At(i));
            if (blk) {
        ...
        MaxBlockEnergy = 0.0;
        fBlocks->Sort();
        emaxblk = static_cast<TritiumShowerBlock*>(fBlocks->At(0));
        if (emaxblk) MaxBlockEnergy = emaxblk->GetEnergy();
    ...
    Int_t nHitBlock = 0;
    fClusters->Sort();
    const TritiumShowerCluster* mainCluster = static_cast<const TritiumShowerCluster*>(fClusters->At(0));
    if (mainCluster) {
        fE = mainCluster->GetE();
        ...
        for (Int_t i=0; i<mainCluster->GetMult(); i++) {
            const TritiumShowerBlock* hitblk = static_cast<const TritiumShowerBlock*>(mainCluster->GetBlocks()->At(i));
    ...
    for (Int_t i=1; i<fClusters->GetLast()+1; i++) {
        const TritiumShowerCluster* cluster = static_cast<const TritiumShowerCluster*>(fClusters->At(i));
        if (cluster) {
            for (Int_t i=0; i<cluster->GetMult(); i++) {
                const TritiumShowerBlock* hitblk = static_cast<const TritiumShowerBlock*>(cluster->GetBlocks()->At(i));
                fNblk[nHitBlock] = hitblk ? hitblk->GetNumber() : -1;
                fEblk[nHitBlock++] = hitblk ? hitblk->GetEnergy() : 0.0;
            }
        }
    ...
    for ( int i=0; i<n_track; i++ ) {
        ...
        new ( (*fTrackProj)[i] ) THaTrackProj(xc,yc,pathl,dx,pad);
    }

    return 0;
}
//_____________________________________________________________________________
Int_t TritiumShower::FineProcess( TClonesArray&  tracks  )
{
